...
/**
   ...
   */function setContentTypeIfUnset(headers,value){!utils.isUndefined(headers)&&utils.isUndefined(headers["Content-Type"])&&(headers["Content-Type"]=value)}var adapter,defaults={adapter:(("undefined"!=typeof XMLHttpRequest||"undefined"!=typeof process&&"[object process]"===Object.prototype.toString.call(process))&&(
...
return function(){function WPQuery(baseURL){!function(instance,Constructor){if(!(instance instanceof Constructor))throw new TypeError("Cannot call a class as a function")}(this,WPQuery),this._baseURL=baseURL}var Constructor,protoProps,staticProps;return Constructor=WPQuery,(protoProps=[{key:"request",value:function(method){var _this=this,resource=arguments.length>1&&void 0!==arguments[1]?arguments[1]:"posts",params=arguments.length>2&&void 0!==arguments[2]?arguments[2]:{},config=arguments.length>3&&void 0!==arguments[3]?arguments[3]:{},data=params,string="/wp-json/wp/v2/".concat(resource);if(data.id&&(string+="/".concat(params.id,"/")),"get"===method){var globals=["fields","embed","method","envelope","jsonp"];string+="?",Object.keys(data).forEach((function(element){var param=element.replace(/\.?([A-Z]+)/g,(function(x,y){return"_".concat(y.toLowerCase())})).replace(/^_/,"");globals.includes(element)&&(param="_".concat(param)),Array.isArray(params[element])?string+="&".concat(param,"=").concat(params[element].join()):"id"!==element&&(string+="&".concat(param,"=").concat(params[element]))}))}return new Promise((function(resolve,reject){axios$1[method](_this._baseURL+string,data,config).then((function(response){return resolve(response)})).catch((function(error){return reject(error.response)}))}))}},{key:"get",value:function(resource,params,config){return this.request("get",resource,params,config)}},{key:"post",value:function(resource,params,config){return this.request("post",resource,params,config)}}])&&_defineProperties(Constructor.prototype,protoProps),staticProps&&_defineProperties(Constructor,staticProps),WPQuery}()}();
